...
exports.handler = async (event) => {
  try {
    ...
    const toolIds = project.Item.toolIds || [];
    ...
    // Batch get tools
    const tools = await dynamoDb.send(new BatchGetCommand({
      RequestItems: {
        [process.env.TOOLS_TABLE]: {
          Keys: toolIds.map(toolId => ({ userId, toolId })),
        },
      },
    }));

    const items = tools.Responses[process.env.TOOLS_TABLE] || [];
